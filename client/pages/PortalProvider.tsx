...
const ALLOWED_PROVIDER_IDS = new Set(partners.map((p) => p.id));
...
export default function PortalProvider() {
  ...
  useEffect(() => {
    const loadProviderData = async () => {
      if (!ALLOWED_PROVIDER_IDS.has(selectedProviderId)) {
        setIsSaved(false);
        return;
      }
                            ...
                            <button
                              ...
                              className={`p-3 rounded-lg border-2 transition-all text-left text-sm font-medium ${
                                selectedAreas.includes(area)
                                  ...
                                  : "border-slate-200 bg-white text-foreground/70 hover:border-slate-300 hover:border-[hsl(193_45%_45%)]"
                              }`}
                            >
                              {area}
                            </button>
